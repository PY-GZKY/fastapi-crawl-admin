...
from app.api.db.mongoCurd import do_find, do_find_select, do_count, do_info
...
async def get_mongo_data(*, request: Request,
    ...
    total_num = await do_count(db)
    return resp_200(data={"queryList":rows,"totalNum":total_num})


# router.add_api_route(methods=['PUT','POST'], path="/file", endpoint=upload_file,
#                      summary="上传文件")

# router.add_api_route(methods=['PUT','POST'], path="/image", endpoint=upload_image,
#                      summary="上传图片")

# router.add_api_route(methods=['PUT','POST'], path="/download/{file_name}", endpoint=download_file,
#                      summary="上传文件")

router.add_api_route(methods=['GET'], path="/mongoData", endpoint=get_mongo_data,
                     summary="mongo数据")
